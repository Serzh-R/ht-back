...
describe('/blogs', () => {
  ...
  it('should create blog with correct input data', async () => {
    ...
    const createResponse = await req.post(SETTINGS.PATH.BLOGS).send(newBlog).expect(201)
    ...
    const getResponse = await req.get(SETTINGS.PATH.BLOGS).expect(200)
  ...
  it('should not create blog with incorrect input data', async () => {
    ...
    const response = await req.post(SETTINGS.PATH.BLOGS).send(invalidBlog).expect(400)
  ...
  it('should return blog by id', async () => {
    ...
    const getResponse = await req.get(`${SETTINGS.PATH.BLOGS}/${createdBlog.id}`).expect(200)

    expect(getResponse.body).toEqual(createdBlog)
  })

  it('should return 404 for not existing blog', async () => {
    await req.get(`${SETTINGS.PATH.BLOGS}/not-existing-id`).expect(404)
  })
  ...
  it('should delete existing blog', async () => {
    ...
    await req.get(`${SETTINGS.PATH.BLOGS}/${createdBlog.id}`).expect(404)
  })

  it('should return 404 when deleting non-existing blog', async () => {
    await req.delete(`${SETTINGS.PATH.BLOGS}/not-existing-id`).expect(404)
  })
})
